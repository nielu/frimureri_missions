...
g_min_limit_travel_distance = 100
g_max_limit_travel_distance = 30000
g_damage_tracker = {}
...
g_objective_types =
{
	...
	repair_vehicle =
	{
		update = function(self, mission, objective, delta_worldtime)
			...
			elseif objective.damaged == false and server.getVehicleSimulating(objective.vehicle_id) then
				...
				if success == false then server.announce("error", "no sign") return true end
	...
	transport_vehicle =
	{
		update = function(self, mission, objective, delta_worldtime)
			...
			for k, obj in pairs(objective.objects) do
				...
				local is_in_zone = server.isInTransformArea(server.getVehiclePos(obj.id), objective.destination.transform, objective.destination.size.x, objective.destination.size.y, objective.destination.size.z)
			...
			if object_delivered_count == object_count then
				if objective.reward_value == nil then objective.reward_value = 3800 end
	...
	transport_object =
	{
		update = function(self, mission, objective, delta_worldtime)
			...
			if object_delivered_count == object_count then
				if objective.reward_value == nil then objective.reward_value = 6400 end
				server.notify(-1, "Delivery Complete", "The consignment has been delivered. Rewarded $"..objective.reward_value..".", 4)
				server.setCurrency(server.getCurrency() + objective.reward_value, server.getResearchPoints() + 1)
				return true
			else
	...
	recover_blackbox = 
	{
		...
		update = function(self,mission,objective,delta_worldtime)
			...
			if is_in_zone then
				if objective.reward_value == nil then objective.reward_value = 4500 end
				server.notify(-1,"Task Complete","Blackbox has been secured. Rewarded $"..objective.reward_value..".",4)
...
g_mission_types =
{
	crashed_vehicle =
	{
		...
		spawn = function(self, mission, location, difficulty_factor, override_transform, min_range, max_range)
			...
			if #zones > 0 then
				...
				local is_transponder = hasTag(location.parameters.capabilities, "transponder") and (math.random(1, 2) == 1)
				...
				if location.objects.vehicle_blackbox ~= nil then
					...
					local blackbox_transform = matrix.multiply(spawn_transform,matrix.translation(math.cos(rad)*dis,0,math.sin(rad)*dis))
					spawned_objects.vehicle_blackbox = spawnObject(blackbox_transform,location,location.objects.vehicle_blackbox,0,nil,all_mission_objects)
				end
				...
				if location.parameters.vehicle_type == "vehicle_type=car" then
					titles = {
						"A road vehicle incident has been reported",
						"A vehicle is reported damaged",
						"A vehicle has gone missing",
						"A vehicle in distress has been sighted",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=train" then
					titles = {
						"A train incident has been reported",
						"A train is reported damaged",
						"A train has gone missing",
						"A train in distress has been sighted",
						"A incident has been reported"
					}
				elseif location.parameters.vehicle_type == "vehicle_type=helicopter" then
					titles = {
						"A helicopter crash has been reported",
						"An aircraft has crashed",
						"A helicopter has gone missing",
						"A crashed helicopter has been sighted",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=plane" then
					titles = {
						"A plane crash has been reported",
						"An aircraft has crashed",
						"A plane has gone missing",
						"A crashed plane has been sighted",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=boat_river" or location.parameters.vehicle_type == "vehicle_type=boat_ocean" then
					titles = {
						"A boat in distress has been reported",
						"A boat has an emergency",
						"A boat has gone missing",
						"A vessel in trouble has been sighted",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=subs_ocean" or location.parameters.vehicle_type == "vehicle_type=subs_river" then
					titles = {
						"A submersible in distress has been reported",
						"A submersible has an emergency",
						"A submersible has gone missing",
						"A incident has been reported"
					}
				else
					titles = {
						"A vehicle incident has been reported",
					}
				end
				...
				if is_transponder then
					...
					mission.desc = "The vehicle has activated its transponder. Use a transponder locator to hone-in on the signal."
		...
		rebuild_ui = function(self, mission)
			if mission.data.state == "rescue" then
				...
				addMarker(mission, createMarker(mission.data.zone_x, mission.data.zone_z, mission.title, mission.desc, mission.data.zone_radius, 8))
			end
		end,
		...
		spawn_location_objectives = function(self, mission)
			...
			if survivor_count > 0 and fire_count > 0 then
				mission.desc = "Extinguish fire and rescue "..survivor_count.." casualties "..mission.data.zone.name.." to a hospital."
				server.notify(-1, "Casualties and Fire", mission.desc, 0)
			elseif survivor_count > 0 then
				mission.desc = "Rescue "..survivor_count.." casualties "..mission.data.zone.name.." to a hospital."
			...
			if has_blackbox then
				...
				mission.desc = table.concat({desc,"find and transport blackbox to the hospital"}," and ")
			end
		end,
	},
	...
	transport =
	{
		...
		spawn = function(self, mission, location, difficulty_factor, override_transform, min_range, max_range)
			...
			local destination_zones = findSuitableZones(dest_parameters, min_range, max_range, is_ocean_zone, override_transform)
			...
			if #source_zones > 0 and #destination_zones > 0 then
				...
				for _, vehicle_object in pairs(mission.data.vehicles) do
					...
					table.insert(mission.objectives, createObjectiveTransportVehicle(vehicle_object, destination_zone, 2000 * reward_distance_factor))
				end

				local survivor_count = 0
				for _, survivor_object in pairs(mission.data.survivors) do
					survivor_count = survivor_count + 1
					table.insert(mission.objectives, createObjectiveTransportCharacter(survivor_object, destination_zone, 2000 * reward_distance_factor))
				end

				local object_count = 0
				for _, object_object in pairs(mission.data.objects) do
					object_count = object_count + 1
					table.insert(mission.objectives, createObjectiveTransportObject(object_object, destination_zone, 1000 * reward_distance_factor))
				end
				...
				elseif vehicle_count > 0 then
					mission.desc = titles[math.random(1, #titles)].." A vehicle needs transporting to "..destination_zone.name
				end
	...
	tow_vehicle =
	{
		...
		build_valid_locations = function(self, playlist_index, location_index, parameters, mission_objects, location_data)
			...
			for _, capability in pairs(parameters.capabilities) do
				if capability == "tow" then is_mission = true end
			end
		...
		spawn = function(self, mission, location, difficulty_factor, override_transform, min_range, max_range)
			...
			if location.parameters.vehicle_type == "vehicle_type=boat_ocean" or location.parameters.vehicle_type == "vehicle_type=boat_river" then
			...
			if #source_zones > 0 and #destination_zones > 0 then
				...
				for _, capability in pairs(location.parameters.capabilities) do
					if capability == "repair" and math.random(1, 3) == 1 then is_repair = true end
				end
				...
				if location.parameters.vehicle_type == "vehicle_type=car" then
					titles = {
						"A car needing repairs requires transport ",
						"A car has reportedly broken down ",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=helicopter" then
					titles = {
						"A helicopter needing repairs requires transport ",
						"A helicopter has failed a systems check and requires transport ",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=plane" then
					titles = {
						"A plane needing repairs requires transport ",
						"An stranded aircraft needs towing "
					}
				elseif location.parameters.vehicle_type == "vehicle_type=boat_river" then
					titles = {
						"A boat in has become stuck ",
						"A stranded boat has requested support ",
					}
				elseif location.parameters.vehicle_type == "vehicle_type=boat_ocean" then
					titles = {
						"A boat has broken down ",
						"A stranded boat has requested support ",
					}
				else
					titles = {
						"A stranded vehicle has been reported ",
						"A vehicle is reported to have broken down ",
				...
				mission.desc = mission.desc .. "Transport "..location.objects.main_vehicle_component.display_name.." "..source_zone.name.." to "..destination_zone.name
	...
	building =
	{
		...
		rebuild_ui = function(self, mission)
			addMarker(mission, createMarker(mission.data.zone_x, mission.data.zone_z, mission.title, mission.desc, mission.data.zone_radius, 5))
		end,
	...
	clear_area = 
	{
		...
		build_valid_locations = function(self, playlist_index, location_index, parameters, mission_objects, location_data)
			if parameters.type == "mission" then
				if mission_objects.vehicle_main ~= nil and hasTag(parameters.capabilities,"obstacle") then
					...
					table.insert(self.valid_locations,{ name=location_data.name,playlist_index = playlist_index,location_index = location_index,data = location_data,objects = mission_objects,parameters = parameters } )
				end
			end
		end,
		...
		spawn = function(self,mission,location,difficulty_factor)
			...
			for _,zone in pairs(availableZones) do
				...
				if location.parameters.vehicle_type == "rock" then
					if 
						zone.parameters.location_type ~= "land_road" and 
						zone.parameters.location_type ~= "railroad_both"
					then
						is_filter=true
					end
			...
			if source_zone ~= nil then
				...
				local spawn_transform = matrix.multiply(spawn_zone.transform,matrix.translation(0,-spawn_zone.size.y * 0.5,0))
				...
				local all_mission_objects = {}
				local spawned_objects = {
					vehicle_main = spawnObject(spawn_transform,location,location.objects.vehicle_main,0,nil,all_mission_objects),
					vehicle_debris = spawnObjects(spawn_transform,location,location.objects.debris,all_mission_objects)
				}
				...
				if location.parameters.vehicle_type=="plane" then
					titles = {
						"A plane has stucked on the runway is reported",
					}
				...
				local reward = 8000
				table.insert(mission.objectives,createObjectiveTransportOutVehicle(spawned_objects.vehicle_main,source_zone,reward))
				for vehicle_id,vehicle_object in pairs(mission.data.vehicle_debris) do
					table.insert(mission.objectives,createObjectiveTransportOutVehicle(vehicle_object,source_zone,reward))
				end
		...
		tick = function(self,mission,delta_worldtime)
			if mission.data.life > 0 then
				if (not mission.is_active) then mission.data.life = mission.data.life - delta_worldtime end
			else
				mission.objectives = {}
			end
		end,
...
function onTick(delta_worldtime)
	...
	if runTime > 10 then server.announce("[def_miss_loc]", "onTick took " .. runTime .. " ms") end
end
	...
	if command == "?mstart" and admin == true then
		if one ~= nil and one ~= "" then
			...
			repeat
				...
				if attempts > 100 then server.announce("[Server]", "No valid locations nearby in 100 attempts.") end
		...
		if command == "?mtest" and admin == true then
			if one ~= nil and one ~= "" then
				...
				repeat
					...
					if attempts > 1000 then server.announce("[Server]", "No valid locations nearby in 1000 attempts.") end
...
function getIncomingDisaster(transform)
	...
	if g_savedata.disasters == nil then return nil end
...
function findSuitableZones(parameters, min_range, max_range, is_ocean_zone, override_transform)
	...
	if is_ocean_zone and math.random(1, 2) == 1 then
		...
		if is_ocean_found then
			...
			local zone_object = {
				name = "in the ocean",
				transform = matrix.multiply(ocean_transform, matrix.translation(math.random(-750, 750), 0, math.random(-750, 750))),
				size = {
					x = 1,
					y = 1,
					z = 1,
